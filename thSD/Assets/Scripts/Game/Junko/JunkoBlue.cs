...
public class JunkoBlue : JunkoBoss
{
    private Vector3[] initPos = new Vector3[2];

    public override void ShootBullet()
    {
        if (JunkoManager.HP > 50) return;

        initPos[0] = new Vector2(Random.Range(-4f, -3f), Random.Range(-1f, 2f));
        initPos[1] = new Vector2(Random.Range(4f, 5f), Random.Range(-1f, 2f));
